        ...
        for (const id of ids) {
            ...
            if (data.length > 0) {
                for (const item of data) {
                    ...
                    const ageRange = roles[0] ? `${roles[0].age_min} - ${roles[0].age_max}` : '';
                    const gender = roles[0]
                        ? roles[0].gender_male && roles[0].gender_female
                            ? 'Any'
                            : roles[0].gender_male
                            ? 'Male'
                            : roles[0].gender_female
                            ? 'Female'
                            : 'Any'
                        : 'Any';
                    updatedData.push({
