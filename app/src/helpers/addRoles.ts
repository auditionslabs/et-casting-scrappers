...
export const rolesSchema = z.object({
    results: z.array(z.object({
        ...
        gender_male: z.number().default(0),
        gender_female: z.number().default(0),
...
export async function addRoles(title: string, description: string, job_url: string) {
    ...
    try {
        ...
        **Your Task:**
        Based on the provided \`Title\`, \`Description\`, and \`Today's Date\`, you will identify and create all necessary casting roles. Your main goal is to produce a concise list of **genuinely distinct** roles, avoiding redundancy. Follow the specific instructions below.
        ...
        **Instructions for Generating Roles:**
        ...
        1.  **Analyze the Input:** Thoroughly read the \`Title\` and \`Description\` to understand the project type, the overall talent needs, and any specific character types mentioned.
        2.  **Identify and Consolidate Distinct Roles:**
            ...
            * **CRUCIAL - PREVENT DUPLICATES:**
                ...
                * If the \`Description\` lists several attributes or slight variations for what is essentially **the same core character type or background need** (e.g., "men 20-30 for cafe scene," "males needed for background, age 25-35," "diner patrons"), **consolidate these into a single, comprehensive role entry.** Combine the descriptions and use the broadest applicable age range if appropriate.
        ...
        3.  **Source of Truth:** All role details must be derived directly from the provided \`Description\`.
        4.  **Gender Specification:**
            * Create separate role entries for Males and Females **only if the \`Description\` clearly specifies distinct needs for each gender within a character type/age group OR if the character type is inherently gendered** (e.g., "King," "Waitress").
        ...
        6.  **Feature Film - Supporting & Extras Default Age:**
            * If the \`Title\` or \`Description\` indicates the project is a **"feature film"**:
        ...
        7.  **Expiration Date Default:**
            * Scan the \`Description\` for any mention of an application deadline or expiration date.
            * If **no expiration date is explicitly provided** in the \`Description\`, calculate it as **30 days from \`Today's Date\`**. Use YYYY-MM-DD format.
